...
class SaprodController extends Controller
{
    ...
    public function newexistenciasphp(Request $request)
    {
        ...
        //dd($instanciaselected->newproductosexistencias);
    ...
    public function list(Request $request)
    {
        ...
        $productos = Saprod::where('comercial',$comercial)
            ->whereRaw("codprod not in (select codprod from saprodsucursal where fk_sucursal=$sucursalid )")->limit(300)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if(isset($producto) and isset($producto->codprod) and $producto->codprod != ''){
            ...
            if($codprod!=''){
                ...
                $producto->exdecimal = isset($request->exdecimal)? $request->exdecimal: 0;
                $producto->activo    = isset($request->activo)   ? $request->activo   : 0;
                $producto->esexento  = isset($request->esexento) ? $request->esexento : 0; ;
                ...
                foreach ($comerciales as $comercial){
                    ...
                    if(isset($product) and isset($product->codprod) and $product->codprod != ''){
                        ...
                        $newprod->preciod   = 0;
                        $newprod->costod    =  0;
                        $newprod->esexento  = (isset($request->esexento))? $request->esexento : 0 ;
                        $newprod->costod2   =  0;
                        $newprod->costod3   =  0;
                        $newprod->comercial = $comercial->id;
                        $newprod->save();
